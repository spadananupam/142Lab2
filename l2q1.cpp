...
void dlist::insert(){
	...
	if (head==NULL){
		...
		tail=temp;//tail to temp then temp is the new tail
	}
}
...
void dlist::deletenode(){
	...
	while (temp->next!=NULL){
		temp=temp->next;
	        t++;//going till last but one
	        }
	temp=tail;//making last but one as tail and storing tail and deleting the old tail
	tail->next=NULL;
	delete temp1;
}
